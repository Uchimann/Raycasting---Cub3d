...
void map_sixthcontrol(t_map *map)
{
	int	i;
    int j;
	char *tmp;
    ...
    if(map->flagcount != 6)
        ft_err("map have not 6 direction programı kapat ");
	...
	//burda bi fonksiyona gidicek o fonksiyonda son okunan yere kadar kontrol edicek. eğer 6 istenen şey dışında ve newline ile space satırı dışında bişey varsa
	//programı bitirecek.
}
